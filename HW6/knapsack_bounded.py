...
def knapsack_bounded(bag_size, items):
    global back
    if bag_size <= 0 or len(items) == 0: # base cases, if bag is full or no item in the store, the value would be 0
        back[bag_size][len(items)] = (-1, 0, (0, 0))
        return 0
    
    weight, value, num, item_idx = items[-1] # get current item properties
    new_items = items[:-1]
    best_choice = (0, 0) # (best_value, best_num)
    if back[bag_size][len(items)] == -1:
        for i in range(num+1):
            if bag_size >= i*weight:
                cur_value = value*i + knapsack_bounded( bag_size - i*weight, new_items )
        ...
        if best_choice == (0, 0): # we dont choose item
            ...
            back[bag_size][len(items)] = (item_idx, weight, best_choice) # ( item_idx, item_weight, best_choice )
    else:
        best_choice = back[bag_size][len(items)][2]
...
def back_trace( bag_size, items, solution ):
    ...
    idx, weight, (best_value, best_num) = back[bag_size][len(items)]
